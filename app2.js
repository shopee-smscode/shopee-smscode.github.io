const BASE_URL = "https://hero-sms-proxy.namakamu.workers.dev"; // [!] PASTIKAN INI SUDAH DIGANTI DENGAN LINK WORKER HERO-SMS ANDA
    ...
    try {
        ...
        if (productsRes.success && productsRes.data.length > 0) {
            ...
            availableProducts.forEach(product => {
                ...
                // PERUBAHAN: Menampilkan lambang $ (Dolar) di daftar Server
                card.innerHTML = `<div class="product-info"><h4>Server ID: ${product.id}</h4><p>Stok: ${product.available}</p></div><div class="product-price">$${product.price}</div>`;
...
function renderOrders() {
    ...
    activeOrders.forEach(order => {
        ...
        // PERUBAHAN: Menampilkan lambang $ (Dolar) di kartu pesanan aktif
        const displayPrice = (order.price && order.price != 0) ? `$${order.price}` : '$ -';
...
function startPollingAndTimer() {
    ...
    pollingInterval = setInterval(async () => {
        ...
        for (let i = 0; i < activeOrders.length; i++) {
            ...
            try { const res = await apiCall(`/orders/${order.id}`); if (res.success) { if (res.data.status === "OTP_RECEIVED") { activeOrders[i].status = "OTP_RECEIVED"; activeOrders[i].otp = res.data.otp_code; saveToStorage(); } else if (res.data.status !== "ACTIVE" && res.data.status !== "PENDING") { activeOrders = activeOrders.filter(o => o.id !== order.id); saveToStorage(); fetchBalance(); } } } catch (e) {}
        }
    }, 5000);
}
...
window.replaceSpecificOrder = async function(orderId, productId) {
    const btn = document.getElementById(`btn-replace-${orderId}`); if (!productId || productId === 'null') return showToast("Pilih server manual."); if (btn) { btn.disabled = true; btn.innerHTML = '<div class="loader"></div>'; }
    try {
        const c = await apiCall('/orders/cancel', 'POST', { id: orderId });
        if (c.success || (c.error && c.error.code === 'NOT_FOUND')) {
            activeOrders = activeOrders.filter(o => o.id !== orderId); const n = await apiCall('/orders/create', 'POST', { product_id: parseInt(productId), quantity: 1 });
            if (n.success) {
...
window.resendSpecificOrder = async function(orderId) {
    const btn = document.getElementById(`btn-resend-${orderId}`); if (btn) { btn.disabled = true; btn.innerHTML = '<div class="loader"></div>'; }
    try {
...
window.cancelSpecificOrder = async function(id, auto = false) {
    ...
    try { const res = await apiCall('/orders/cancel', 'POST', { id: id }); if (res.success || (res.error && res.error.code === 'NOT_FOUND')) { activeOrders = activeOrders.filter(o => o.id !== id); saveToStorage(); fetchBalance(); if(auto) showToast("Otomatis batal (waktu sisa 10 menit)"); } else { showToast("Gagal dibatalkan."); if (btnCancel) btnCancel.disabled = false; } } catch (e) { if (btnCancel) btnCancel.disabled = false; }
};
...
window.onload = () => {
    ...
    initNotesSync();
    const saved = sessionStorage.getItem('savedAccountName'); if (saved) loginAccount(saved); else fetchAccounts();
};
