...
// --- FUNGSI NORMALISASI NOMOR (STANDAR 62) ---
...
window.addEventListener('popstate', (e) => {
    const blM = document.getElementById('blacklistModal');
...
function initUsedNumbersSync() {
    db.ref('used_numbers/hero_sms').on('value', snapshot => {
        ...
        if (snapshot.exists()) {
            snapshot.forEach(child => {
                if (child.val().phone) {
                    ...
                    usedNumbersDB.add(normalPhone);
                    totalBlacklist++;
                    
                    let op = getProviderName(normalPhone);
        ...
        for (let op in operatorCounts) { 
            breakdownText += `<span style="display:inline-block; background:rgba(0,0,0,0.05); padding:3px 10px; border-radius:12px; margin:3px; font-size:11px; font-weight:bold; color:var(--text-primary); border: 1px solid var(--border-color);">${op}: ${operatorCounts[op]}</span>`; 
        }
        
        let breakdownDiv = document.getElementById('operatorBreakdown');
        if(!breakdownDiv) {
            breakdownDiv = document.createElement('div');
            breakdownDiv.id = 'operatorBreakdown';
            breakdownDiv.style.marginTop = "15px";
            breakdownDiv.style.textAlign = "center";
            const targetParent = document.querySelector('#blacklistModal .modal-content div[style*="text-align: center"]');
            if(targetParent) targetParent.appendChild(breakdownDiv);
        }
...
function recordStat(type) {
    ...
    const statRef = db.ref(`stats/hero_sms/${today}/${type}`);
...
function loadStatsData() {
    ...
    const sFailed = document.getElementById('statFailed');
    
    if(sSuccess) sSuccess.innerText = "...";
    if(sFailed) sFailed.innerText = "...";
    
    db.ref(`stats/hero_sms/${selectedDate}`).once('value', snap => {
        const data = snap.val();
        if(sSuccess) sSuccess.innerText = data?.success || 0;
        if(sFailed) sFailed.innerText = data?.failed || 0;
    });
}
