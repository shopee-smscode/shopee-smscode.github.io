...
async function apiCall(endpoint, method = 'GET', urlParams = "") {
    ...
    try {
        ...
        if (err.name === 'AbortError') {
            return { success: false, message: "Koneksi lambat (Timeout)" };
        }
...
window.addEventListener('online', () => { showToast("🌐 Online", "success"); startPolling(); initApp(); });
...
async function fetchServices() {
    ...
    try {
        ...
        if (res.success || res.status) {
            ...
            btnText.innerText = "Gagal Memuat Layanan";
        }
    } catch (e) {
        btnText.innerText = "Error Layanan";
    }
}
        ...
        try {
            ...
            if (hasSms) {
                let stackHtml = smsArray.map((msg) => {
                    ...
                    let displayCode = extracted ? extracted[0] : code; 
