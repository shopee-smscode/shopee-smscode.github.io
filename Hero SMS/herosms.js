...
function renderHistory() {
    ...
    orderHistory.forEach(item => {
        ...
        card.innerHTML = `
            ...
            ${item.status === 'SUKSES' || item.status === 'MINTA ULANG' ? `<div style="background: var(--otp-bg); border: 1px dashed ${statusColor}; color: ${statusColor}; padding: 6px; text-align: center; border-radius: 6px; font-weight: 900; letter-spacing: 4px; font-size: 18px; text-shadow: 0 0 10px rgba(150,212,0,0.3);">${item.otp}</div>` : ''}
        `;
